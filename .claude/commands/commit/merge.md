...
**重要**: Worktree内で作業している場合、削除前に必ずメインリポジトリへ移動すること。
移動しないとシェルのcwdが無効になり、以降のコマンドが全て失敗する。

9. **メインブランチに戻る＆更新**
    ```bash
    # 現在のWorktreeパスを記録（削除用）
    CURRENT_WORKTREE=$(pwd)
    MAIN_REPO=$(git worktree list | head -1 | awk '{print $1}')

    # ★ 必ずメインリポジトリへ移動してからWorktreeを操作する
    cd "$MAIN_REPO"
    git checkout main
    git pull
    ```
...
10. **Worktree削除**
    ```bash
    # メインリポジトリにいることを確認してから削除
    git worktree remove "$CURRENT_WORKTREE" 2>/dev/null || true
    ```
...
13. **Issueクローズ**（PRマージで自動クローズされなかった場合）
    ```bash
