...
1. **setup-labels の失敗/スキップでも初期化を続行する**（理由: gh 未認証・remote 未設定は
   インストール時点で正常にありうる状態のため）。案内メッセージは必須。
   終了コードで区別する（0=作成 / 2=スキップ（要対応）/ 1=エラー）
2. **初期化シーケンス全体（worktree-init ラッパー）の失敗でも installer を落とさない**
   （理由: installer の主目的はファイル配置であり、それは既に完了している。
   初期化はいつでも再実行できる）。**警告と再実行コマンドの提示は必須**。
   再実行コマンドは「インストール先に実在するパス」を出すこと
   （既存 `.claude/skills/` があるとラッパーは配置されないため、決め打ちにしない）

   ※ 検証（1周目）で `|| warn` の実スコープが 1 より広いことが指摘されたため、
   スコープを明示して承認範囲に含めた。無言の `|| true` は引き続き禁止。

**このリスト以外の握り潰しは禁止。**

## スコープ外（#122 送り）

- `--force` 時に `shared_data_path` / `path_type` / `storage_type` も上書き前の値へ復元すること。
  created_at を復元したことで「テンプレートの固定値」という差し替えのサインが消えたため、
  データ保存先の silent reset が気づきにくくなった（検証2周目の指摘）。
  `worktree-config.json` の全上書き自体は main からの既存挙動であり本 issue の導入ではない
