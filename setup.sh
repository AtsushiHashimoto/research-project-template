...
# Generate .devcontainer/.env for stable volume naming
# devcontainer CLI overrides COMPOSE_PROJECT_NAME, so we use PROJECT_SLUG instead.
PROJECT_SLUG=$(echo "$PROJECT_NAME" | tr '[:upper:]' '[:lower:]' | tr ' ' '-' | sed 's/[^a-z0-9_-]//g')
cat > .devcontainer/.env << EOF
# Auto-generated by setup.sh — keeps claude-code-config volume name stable across rebuilds
# devcontainer CLI overrides COMPOSE_PROJECT_NAME, so we use a dedicated variable.
PROJECT_SLUG=${PROJECT_SLUG}
EOF
echo "Generated .devcontainer/.env (PROJECT_SLUG=${PROJECT_SLUG})"
